...
-- 계정 생성
book_rental_systemCREATE USER 'lunchtribeuser'@'%' IDENTIFIED BY 'lunchtribeuser';
...
ALTER TABLE `notification_history` ADD CONSTRAINT `PK_NOTIFICATION_HISTORY` PRIMARY KEY (
	`notification_history_code`
);

ALTER TABLE `restaurant_bookmark` ADD CONSTRAINT `PK_RESTAURANT_BOOKMARK` PRIMARY KEY (
	`restaurant_bookmark_code`
);
...
ALTER TABLE `notification_send` ADD CONSTRAINT `PK_NOTIFICATION_SEND` PRIMARY KEY (
	`notification_send_code`
);

ALTER TABLE `restaurant_likes` ADD CONSTRAINT `PK_RESTAURANT_LIKES` PRIMARY KEY (
	`restaurant_likes_code`
);
...
ALTER TABLE `notification_status` ADD CONSTRAINT `PK_NOTIFICATION_STATUS` PRIMARY KEY (
